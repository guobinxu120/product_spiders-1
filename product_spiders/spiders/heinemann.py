...
class dubaidutyfreeSpider(scrapy.Spider):
	...
	# use_selenium = False
	domain = 'http://www.heinemann-dutyfree.com/frankfurt_en/'
	total_count = 0
...
###########################################################
	...
	def start_requests(self):
		yield scrapy.Request(url='http://www.heinemann-dutyfree.com/frankfurt_en/', callback=self.parse_category)

	def parse_category(self, response):
		cats = response.xpath('//nav[@id="navigation"]/ul/li/ul/li/a[@class="nav__items__link"]/@href').extract()
		for cat in cats:
			N = cat.split('-')[-1]
